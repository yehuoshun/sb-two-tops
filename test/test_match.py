...
def main():
    ...
    # 测试区域：左上角 200x200
    roi = gray[0:200, 0:200]
    result_roi = cv2.matchTemplate(roi, template, cv2.TM_CCOEFF_NORMED)
    _, max_val0, _, max_loc0 = cv2.minMaxLoc(result_roi)
    print(f"左上角 200x200: 最高置信度={max_val0:.4f} 位置=({max_loc0[0]}, {max_loc0[1]})")

    # 测试区域：右上角 400x200（battle.py 实际使用的 SEARCH_REGION）
    roi = gray[0:200, w-400:w]
    result_roi = cv2.matchTemplate(roi, template, cv2.TM_CCOEFF_NORMED)
    _, max_val2, _, max_loc2 = cv2.minMaxLoc(result_roi)
    real_x = (w - 400) + max_loc2[0]
    print(f"右上角 400x200: 最高置信度={max_val2:.4f} 位置=({real_x}, {max_loc2[1]})")
    ...
    print(f"右下角 400x200: 最高置信度={max_val3:.4f} 位置=({real_x3}, {max_loc3[1] + h - 200})")
